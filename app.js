...
(() => {
    ...
    /* ===================================================================
     * Deploy Files — bulk upload to all panels
     * =================================================================== */

    // Track uploaded files
    let __deployFiles = [];
    // Track panel list for checkboxes
    let __deployPanels = [];
    ...
    function renderDeployFileList() {
        const list = document.getElementById("adm-file-list");
        const btn  = document.getElementById("adm-deploy-btn");
        if (!list) return;

        if (!__deployFiles.length) {
            list.classList.add("adm-hidden");
            if (btn) btn.disabled = true;
            return;
        }

        list.classList.remove("adm-hidden");
        list.innerHTML = __deployFiles.map((f, i) => `
            <div class="adm-file-item">
                <span class="adm-file-item-name" title="${escapeHtml(f.name)}">📄 ${escapeHtml(f.name)}</span>
                <span class="adm-file-item-size">${escapeHtml(formatFileSize(f.size))}</span>
                <button class="adm-file-item-remove" data-file-idx="${i}" title="Remove">×</button>
            </div>
        `).join("");

        list.querySelectorAll(".adm-file-item-remove").forEach(btn => {
            btn.addEventListener("click", e => {
                const idx = Number(e.currentTarget.dataset.fileIdx);
                __deployFiles.splice(idx, 1);
                renderDeployFileList();
            });
        });

        if (btn) btn.disabled = __deployFiles.length === 0;
    }

    function addFilesToDeploy(fileList) {
        for (const f of fileList) {
            // Avoid duplicates by name
            if (!__deployFiles.some(x => x.name === f.name)) {
                __deployFiles.push(f);
            }
        }
        renderDeployFileList();
    }

    async function loadDeployPanelCheckboxes() {
        const container = document.getElementById("adm-panel-checkboxes");
        if (!container) return;

        const res = await api("/api/panels?status=0");
        if (!res || !res.ok) {
            container.innerHTML = '<span style="color:var(--adm-danger);font-size:13px">Failed to load panels</span>';
            return;
        }

        __deployPanels = (res.data && res.data.panels) || [];
        if (!__deployPanels.length) {
            container.innerHTML = '<span style="color:var(--adm-text-muted);font-size:13px">No panels configured</span>';
            return;
        }

        container.innerHTML = __deployPanels.map(p => `
            <label class="adm-panel-check-label">
                <input type="checkbox" data-panel-id="${escapeHtml(p.id)}" checked>
    ...
    function getSelectedPanelIds() {
        const checks = document.querySelectorAll("#adm-panel-checkboxes input[type=checkbox]:checked");
        return Array.from(checks).map(c => c.dataset.panelId).filter(Boolean);
    }

    async function deployFiles() {
        if (!__deployFiles.length) {
            showToast("No files to deploy.", "warn");
            return;
        }

        const selectedPanels = getSelectedPanelIds();
        if (!selectedPanels.length) {
            showToast("No panels selected.", "warn");
            return;
        }

        const remotePath = (document.getElementById("adm-remote-path") || {}).value || "";
        const restartAfter = (document.getElementById("adm-restart-after") || {}).checked || false;

        if (!confirm(`Deploy ${__deployFiles.length} file(s) to ${selectedPanels.length} panel(s)${restartAfter ? " + restart" : ""}?`)) {
            return;
        }
        ...
        const resultsEl = document.getElementById("adm-deploy-results");
        if (resultsEl) {
            resultsEl.classList.remove("adm-hidden");
            resultsEl.innerHTML = '<div style="color:var(--adm-text-2);font-size:13px;padding:8px">Uploading…</div>';
        }

        try {
            const formData = new FormData();
            for (const f of __deployFiles) {
            ...
            const r = await fetch("/api/panels/deploy", {
                method: "POST",
            ...
            if (resultsEl) {
                resultsEl.innerHTML = `<div style="color:var(--adm-danger);padding:8px">Error: ${escapeHtml(String(err.message || err))}</div>`;
            }
            showToast("Deploy failed: " + (err.message || err), "error");
        }
    }

    function renderDeployResults(data) {
        const el = document.getElementById("adm-deploy-results");
        if (!el) return;

        if (!data || data.status !== "success") {
            const msg = (data && data.message) || "Unknown error";
            el.innerHTML = `<div class="adm-deploy-summary is-fail">✗ Deploy failed: ${escapeHtml(msg)}</div>`;
            return;
        }

        const summary = data.summary || {};
        const results = data.results || {};
        const restartResults = data.restartResults || {};

        const summaryClass = summary.panelsFailed === 0 ? "is-ok"
            : summary.panelsOk === 0 ? "is-fail" : "is-partial";
        const summaryIcon = summary.panelsFailed === 0 ? "✅" : summary.panelsOk === 0 ? "✗" : "⚠";

        let html = `<div class="adm-deploy-summary ${summaryClass}">
            ${summaryIcon} ${summary.panelsOk}/${summary.panels} panel berhasil
            · ${summary.filesPerPanel} file per panel
            ${Object.keys(restartResults).length ? `· Restart: ${Object.values(restartResults).filter(r => r.ok).length}/${Object.keys(restartResults).length} ok` : ""}
        </div>`;

        for (const [panelId, panelRes] of Object.entries(results)) {
            const allOk = panelRes.failed === 0;
            const rowClass = allOk ? "is-ok" : "is-fail";
            const icon = allOk ? "✅" : "⚠";
            const restartInfo = restartResults[panelId]
                ? (restartResults[panelId].ok ? " · 🔄 restarted" : ` · restart failed: ${escapeHtml(restartResults[panelId].message)}`)
                : "";

            html += `<div class="adm-deploy-result-row ${rowClass}">
                <div class="adm-deploy-result-panel">
                    ${icon} ${escapeHtml(panelRes.name || panelId)}
                    <span style="font-weight:400;color:var(--adm-text-3);font-size:11px">${escapeHtml(restartInfo)}</span>
                </div>
                <div class="adm-deploy-result-files">`;

            for (const [filename, fileRes] of Object.entries(panelRes.files || {})) {
                const fClass = fileRes.ok ? "is-ok" : "is-fail";
                const fIcon = fileRes.ok ? "✓" : "✗";
                html += `<div class="adm-deploy-result-file ${fClass}">
                    ${fIcon} ${escapeHtml(filename)}
                    ${!fileRes.ok ? `<span style="color:var(--adm-text-3)">${escapeHtml(fileRes.message)}</span>` : ""}
                </div>`;
            }
        ...
        // Show toast summary
        if (summary.panelsFailed === 0) {
            showToast(`✅ Deploy selesai — ${summary.panelsOk} panel updated`, "success");
        } else {
            showToast(`⚠ Deploy partial — ${summary.panelsOk}/${summary.panels} ok`, "warn");
        }
    }

    function initDeploy() {
        const dropZone   = document.getElementById("adm-drop-zone");
        const fileInput  = document.getElementById("adm-file-input");
        const deployBtn  = document.getElementById("adm-deploy-btn");

        if (!dropZone || !fileInput || !deployBtn) return;
        ...
        fileInput.addEventListener("change", () => {
            if (fileInput.files && fileInput.files.length) {
                addFilesToDeploy(fileInput.files);
                fileInput.value = "";
            }
        });
        ...
        dropZone.addEventListener("drop", e => {
            ...
            if (e.dataTransfer && e.dataTransfer.files.length) {
                addFilesToDeploy(e.dataTransfer.files);
            }
        });

        // Deploy button
        deployBtn.addEventListener("click", deployFiles);

        // Load panel checkboxes
        loadDeployPanelCheckboxes();
    }
