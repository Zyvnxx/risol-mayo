...
# ---------------------------------------------------------------------------
# Bulk file deploy — write one or more files to ALL configured panels
# simultaneously via the Pterodactyl Client API file-write endpoint.
# ---------------------------------------------------------------------------

def _ptero_write_file(panel, remote_path, content_bytes):
    """Write ``content_bytes`` to ``remote_path`` on a single panel.

    Returns (panel_id, ok, message)."""
    pid = panel["id"]
    if not (panel.get("panelUrl") and panel.get("serverId") and panel.get("clientApiKey")):
        return pid, False, "Panel not configured."
    ...
    try:
        ...
        return pid, False, "Request failed: " + str(e)

    if 200 <= r.status_code < 300:
        return pid, True, "ok"
    return pid, False, "HTTP {}: {}".format(r.status_code, r.text[:200])


@app.route("/api/panels/deploy", methods=["POST"])
def api_panels_deploy():
    """
    Upload one or more files and write them to every configured panel.

    Accepts multipart/form-data with:
      - One or more file fields (any field name, multiple files ok)
      - Optional JSON field  ``options``  with:
          {
            "panelIds":    ["id1", "id2"],  // subset of panels; omit = all
            "remotePath":  "cogs/",         // target directory on server
            "restartAfter": true            // send restart signal after upload
          }

    Returns per-panel results.
    """
    if not _check_password():
        return _unauthorized()
    ...
    remote_dir = str(options.get("remotePath") or "").strip().rstrip("/")
    restart_after = bool(options.get("restartAfter", False))
    panel_ids = options.get("panelIds")  # None = all panels

    # --- Collect target panels ---
    all_panels = _collect_panels()
    if panel_ids and isinstance(panel_ids, list):
        target_panels = [p for p in all_panels if p["id"] in panel_ids]
    else:
    ...
    # Read all file contents upfront (before threading)
    file_payloads = []
    for f in files:
        ...
        file_payloads.append((filename, remote_path, content))

    # --- Deploy to all panels in parallel ---
    results = {}  # panel_id -> {ok, failed, errors}

    def deploy_to_panel(panel):
        pid = panel["id"]
        panel_result = {"name": panel["name"], "files": {}}
        for filename, remote_path, content in file_payloads:
            _, ok, msg = _ptero_write_file(panel, remote_path, content)
            panel_result["files"][filename] = {"ok": ok, "message": msg}
        # Count outcomes
        panel_result["ok"] = sum(1 for v in panel_result["files"].values() if v["ok"])
        panel_result["failed"] = sum(1 for v in panel_result["files"].values() if not v["ok"])
        return pid, panel_result

    deploy_results = list(_EXECUTOR.map(deploy_to_panel, target_panels))
    for pid, res in deploy_results:
        results[pid] = res

    # --- Restart panels that had all files succeed (if requested) ---
    restart_results = {}
    if restart_after:
        def restart_panel(panel):
            pid = panel["id"]
            if not results.get(pid, {}).get("failed", 1) == 0:
                return pid, False, "Skipped (upload had failures)"
            ...
            try:
                ...
                return pid, ok, ("ok" if ok else "HTTP " + str(r.status_code))
            except requests.RequestException as e:
                return pid, False, str(e)

        for pid, ok, msg in _EXECUTOR.map(restart_panel, target_panels):
            restart_results[pid] = {"ok": ok, "message": msg}
            if ok:
                _invalidate_status(pid)

    # --- Summary ---
    total_panels = len(target_panels)
    total_files = len(file_payloads)
    panels_ok = sum(1 for r in results.values() if r["failed"] == 0)
    ...
    return jsonify({
        ...
        "summary": {
            "panels": total_panels,
            "panelsOk": panels_ok,
            "panelsFailed": total_panels - panels_ok,
            "filesPerPanel": total_files,
        },
        "results": results,
